...
describe('EarthquakeStore', () => {
	...
	it('invalidates pending snapshot on reconnect so deferred old response does not count as fresh baseline', async () => {
		...
		const streamQuake = makeEarthquake({ id: 1, place: 'Fresh SSE Quake', updated_at_ms: 50 });
		stream.emit('new', streamQuake);
		expect(store.earthquakes.get(1)?.place).toBe('Fresh SSE Quake');
		...
		resolveOldSnapshot(
			new Response(
				JSON.stringify({
					earthquakes: [
						makeEarthquake({ id: 1, place: 'Stale Pre-Reconnect Quake', updated_at_ms: 10 })
					]
				})
			)
		);
		...
		resolveNewSnapshot(
			new Response(
				JSON.stringify({
					earthquakes: [makeEarthquake({ id: 1, place: 'Fresh SSE Quake', updated_at_ms: 50 })]
				})
			)
		);
		await settle();
